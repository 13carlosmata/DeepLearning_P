...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        self.cost_verification = self._get_cost(logits, cost, cost_kwargs)
...
class Trainer(object):
    ...
    def _initialize(self, training_iters, output_path, restore):
        ...
        tf.summary.scalar('loss_verification',self.net.cost_verification)
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1, restore=False, write_graph=False):
        ...
        with tf.Session() as sess:
			...
			self.output_minibatch_stats_verification(sess, summary_writer, step, test_x, util.crop_to_shape(test_y, pred_shape))
    ...
    def output_minibatch_stats_verification(self, sess, summary_writer, step, batch_x, batch_y):
        # Calculate batch loss and accuracy
	self.net.cost_verification = self.net.cost
        summary_str, loss_ver, acc_ver, predictions_ver = sess.run([self.summary_op, 
                                                            self.net.cost_verification, 
                                                            self.net.accuracy, 
                                                            self.net.predicter], 
                                                           feed_dict={self.net.x: batch_x,
                                                                      self.net.y: batch_y,
